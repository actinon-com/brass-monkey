...
export async function inspectModel(manager: InstanceManager, input: InspectModelInput) {
  ...
  for (const [fieldName, fieldData] of Object.entries(rawFields as Record<string, any>)) {
    const properties: string[] = [];

    // Compress behavioral flags into the "properties" array
    if (fieldData.required) properties.push('required');
    if (fieldData.readonly) properties.push('readonly');
    if (fieldData.company_dependent) properties.push('company-dependent');
    if (fieldData.store === false) properties.push('not-stored');
