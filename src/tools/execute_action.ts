 ...
 */
const UNSAFE_STATES: Record<string, string> = {
 ...
 */
export async function executeAction(manager: InstanceManager, input: ExecuteActionInput) {
  ...
  for (const node of flat) {
    if (
      ...
      throw new Error(
        `Action ${node.id} ("${node.name}") is declared against model "${node.model_technical}", ` +
        `but was invoked with model "${model}". Re-check the action with get_action before running it.`
      );
    }
  }

  const declaredStates = flat.map((n) => n.state).filter(Boolean) as string[];
  const allDeclarative =
    declaredStates.length > 0 && declaredStates.every((s) => DECLARATIVE_STATES.includes(s));
  const snapshotFields = Array.from(
    new Set(flat.map((n) => n.update_field).filter(Boolean) as string[])
  );
  ...
  const before = await guard.captureSnapshot(
    model,
    targets.map((t) => t.id),
    snapshottable ? snapshotFields : null,
    snapshotReason
  );
 ...
 */
async function expandActionTree(
  client: any,
  actionId: number,
  fields: string[],
  ...
  if (!recs || recs.length === 0) {
    throw new Error(
      `Server action not found with ID ${actionId}. Use get_action to confirm the action exists ` +
  ...
  if (Array.isArray(rec.update_field_id)) {
    node.update_field = await resolveTechnicalName(
      client,
      'ir.model.fields',
      rec.update_field_id[0],
      'name'
    );
  }
...
async function resolveTechnicalName(
  client: any,
  model: string,
  id: number,
  column: string
...
function flatten(node: ActionNode): ActionNode[] {
