 ...
 */
async function fetchSingleRecordDetail(client: any, instanceAlias: string, model: string, resId: number, flags: any): Promise<any> {
  ...
  let activeFields: string[] = [...metadata.baseFields];

  if (flags.show_extended) activeFields.push(...Object.keys(buckets.extended));
  if (flags.show_computed) activeFields.push(...Object.keys(buckets.computed));
  if (flags.show_related) activeFields.push(...Object.keys(buckets.related));
  if (flags.show_relationships) activeFields.push(...Object.keys(buckets.relational));
  if (flags.show_lines) activeFields.push(...Object.keys(buckets.lines));
  if (flags.show_all_fields) {
    activeFields.push(
      ...Object.keys(buckets.extended),
      ...Object.keys(buckets.computed),
      ...Object.keys(buckets.related),
      ...Object.keys(buckets.relational),
      ...Object.keys(buckets.lines)
    );
  }
  ...
  if (!records || records.length === 0) throw new Error(`Record ID ${resId} not found on ${model}`);
  ...
  if (flags.show_chatter) {
    try {
      ...
      record._chatter = messages.map((m: any) => ({
        date: m.date,
  ...
  if (flags.show_security) {
    try {
      ...
      record._security = access.reduce((acc: any, a: any) => {
 ...
 */
export async function getRecords(manager: InstanceManager, input: GetRecordsInput) {
  const { model, res_ids = [], xml_ids = [], instance_alias, ...flags } = input;
  const client = await manager.getClient(instance_alias);
  const alias = instance_alias || 'default';

  const resolvedIds: { id: number; xmlId?: string }[] = [];
