...
export class SkillGuard {
  ...
  validateAccess(toolName: string, args: any): void {
    if (EXEMPT_TOOLS.includes(toolName)) return;
    ...
    if (requiredSkill && !this.activatedSkills.has(requiredSkill)) {
      throw new Error(
        `DOMAIN_LOCKED: Access to model '${model}' is locked. You must first activate the '${requiredSkill}' skill to internalize the expert domain rules for this operation.`
      );
    }
  }
}
