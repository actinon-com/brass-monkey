...
export class ExecutionGuard {
  ...
  async resolveTargets(
    ...
    if (ids.length === 0) {
      if (!allowEmpty) {
        throw new Error(
          `No target records supplied for ${model}. An empty recordset is refused by default ` +
    ...
    const found = new Map<number, string>(
      (records || []).map((r: any) => [r.id, r.display_name || r.name || `ID ${r.id}`])
    );
    const missing = ids.filter((id) => !found.has(id));
    if (missing.length > 0) {
      throw new Error(
        `Target records do not exist on ${model}: ${missing.join(', ')}. ` +
        `Verify the ids with search_records before executing.`
      );
    }

    return ids.map((id) => ({ id, display_name: found.get(id) as string }));
  }
  ...
  assertAcknowledged(report: PreflightReport, acknowledged: boolean): void {
    if (!report.requires_acknowledgement || acknowledged) return;

    throw new Error(
      `Execution refused: this ${report.kind.replace('_', ' ')} is classified as ` +
   ...
   */
  async availableFields(model: string, candidates: string[]): Promise<string[]> {
    try {
  ...
  async recordExecution(params: {
    ...
    await this.audit.logSystemEvent(
      `Executed ${kind} "${label}" on ${model} [${targetLabel}]: ${justification}`,
      'warning'
    );
  ...
  private formatExecutionSummary(
    ...
    if (before.data && Object.keys(before.data).length > 0) {
      const rows = Object.entries(before.data)
        .map(
          ([id, fields]) =>
            `<li><code>${id}</code>: ${Object.entries(fields)
 ...
 */
export function coerceIdList(val: unknown): unknown {
 ...
 */
export function coerceKwargs(val: unknown): unknown {
  if (val === undefined || val === null) return {};
  if (typeof val === 'string') {
    const trimmed = val.trim();
    if (trimmed === '') return {};
