...
export class OdooOrchestrator {
   ...
   */
  async fetchTranslationMatrix(model: string, resIds: number[], fieldNames: string[]) {
    if (!fieldNames.length || !resIds.length) return {};
    ...
    try {
      ...
      const matrix: Record<number, Record<string, Record<string, any>>> = {};
   ...
   */
  async applyBroadcastWrite(model: string, resId: number | null, values: Record<string, any>, transFields: string[]) {
    const mainWrite: Record<string, any> = {};
    const translationWrites: Record<string, Record<string, any>> = {};

    const langs = await this.client.executeKw('res.lang', 'search_read', [[['active', '=', true]]], { fields: ['code'] });
    const langCodes = langs.map((l: any) => l.code);
    for (const lcode of langCodes) translationWrites[lcode] = {};
        ...
        if (Array.isArray(val)) {
          ...
          for (const bucket of val) {
            ...
            if (targetLangs.length === 0) {
              ...
              for (const lcode of langCodes) translationWrites[lcode][fname] = bucket.value;
            } else {
              for (const lcode of targetLangs) {
                if (translationWrites[lcode]) translationWrites[lcode][fname] = bucket.value;
              }
            }
          }
        } else {
          ...
          mainWrite[fname] = val;
          for (const lcode of langCodes) translationWrites[lcode][fname] = val;
        }
      } else {
        mainWrite[fname] = val;
      }
   ...
   */
  async enrichRelationalLabels(model: string, records: any[]) {
   ...
   */
  async resolveFieldValues(model: string, values: Record<string, any>) {
    const fRecords = await this.client.executeKw('ir.model.fields', 'search_read', [[['model_id.model', '=', model]]], {
      fields: ['name', 'ttype', 'relation']
    });
    const fieldMap = fRecords.reduce((acc: any, f: any) => {
      acc[f.name] = f;
      return acc;
    }, {});

    const resolvedValues: Record<string, any> = { ...values };
    ...
    for (const [fname, val] of Object.entries(values)) {
      ...
      if (field.ttype === 'many2one' && typeof val === 'string') {
        ...
        if (matches.length === 1) {
          ...
          const options = matches.map((m: any) => `${m[1]} (ID: ${m[0]})`).join(', ');
