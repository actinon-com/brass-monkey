import { z } from 'zod';
import { ExecutionGuard, coerceIdList, coerceKwargs, } from '../services/execution-guard.js';
/**
 ...
 */
export async function executeMethod(manager, input) {
    const parsedInput = ExecuteMethodSchema.parse(input);
    const { model, method, ids, kwargs, justification, dry_run, acknowledge_unsafe, skip_view_validation, allow_empty_recordset, instance_alias, } = parsedInput;
    // 1. Shape check before anything reaches the wire.
    if (!/^[A-Za-z_][A-Za-z0-9_]*$/.test(method)) {
        throw new Error(`"${method}" is not a valid Odoo method name. Expected an identifier such as "action_confirm".`);
    }
    // 2. Hard deny-list. Not overridable, by design.
    if (method.startsWith('_')) {
        throw new Error(`Refused: "${method}" is a private method. Brass-Monkey only calls documented, ` +
            `user-invocable workflow methods.`);
    }
    const deniedReason = DENIED_METHODS[method];
    if (deniedReason) {
        throw new Error(`Refused: "${method}" is an ORM primitive and is not callable through execute_method — ` +
    ...
    if (!matchesConvention) {
        reasons.push(`"${method}" does not follow the ${ALLOWED_PREFIXES.join('/')} convention Odoo uses for ` +
    ...
    if (foreignKwargs.length > 0) {
        reasons.push(`keyword arguments beyond "context" were supplied (${foreignKwargs.join(', ')}), which is ` +
    ...
    if (!viewMatch.found && !skip_view_validation) {
        throw new Error(`Execution refused: no <button name="${method}"> bound to an object action was found in the ` +
    ...
    const before = await guard.captureSnapshot(model, targetIds, snapshottable ? stateFields : null, report.snapshot_unavailable_reason);
    // 6. Call the method against the recordset.
    const result = await client.executeKw(model, method, [targetIds], kwargs);
    const after = await guard.captureSnapshot(model, targetIds, snapshottable ? stateFields : null, report.snapshot_unavailable_reason);
    ...
    return {
        ...
        result: result ?? null,
        follow_up_action_note: result && typeof result === 'object' && !Array.isArray(result)
 ...
 * check as core ones.
 */
async function findButtonInViews(client, model, method) {
